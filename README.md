...
| :------------------------------- | :---------------------------------------------------- | :---------- | :-------------------------------------------------------------------------------------------------------------------------------------------- |
| **! REQUIRED !** `t1EndpointUrl` | `string`                                              | -           | URL of the endpoint to retrieve the t1 value from.                                                                                            |
| **! REQUIRED !** `t1CalcFn`      | `(httpResponse: Response) => Promise<number \| null>` | -           | Function used to process the retrieved t1 value.                                                                                              |
...
| `maxSyncAttempts`                | `number`                                              | `6`         | Maximum amount of allowed t1 fetch requests when calling `.sync()` (i.e. the amount of times `.generateData()` is called).                    |
| `requiredOkSyncAttempts`         | `number`                                              | `4`         | Required amount of successful t1 data fetches per `.sync()` call. (i.e. the amount of `.generateData()` calls that must return an OK result). |
